...
function displayCategories() {
    global $dbConn;
    $sql = "SELECT * FROM tp_categories ORDER BY category ASC";
...
function displayColors() {
    global $dbConn;
    $sql = "SELECT DISTINCT flowerColor FROM tp_flowers";
    $stmt = $dbConn->prepare($sql);
    $stmt->execute();
    $record = $stmt->fetchALL(PDO::FETCH_ASSOC);
...
function filterProducts() {
    ...
    global $temp;
        if (!empty($_GET['searchForm'])) {
            ...
            $np = array();
            $sql = "SELECT * FROM tp_flowers NATURAL JOIN tp_categories WHERE 1 ";
...
function displayAllProducts(){
    ...
    $sql = "SELECT * FROM tp_flowers ORDER BY flowerName";
    $stmt = $dbConn->prepare($sql);
    $stmt->execute();
...
function getFlowerInfo($flowerId) {
    ...
    $sql = "SELECT * FROM tp_flowers WHERE flower_Id = $flowerId";
    $stmt = $dbConn->prepare($sql);
    $stmt->execute();
